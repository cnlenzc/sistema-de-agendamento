...
class AgendamentoAPI(APITestCase):
    ...
    def test_create_hora_inicial_maior_hora_final(self):
        response = self.client.post(
            '/agendamento/',
            data=reg_hora_inicial_maior_hora_final['in'])
        ...
        self.assertEqual(
            response.data,
            reg_hora_inicial_maior_hora_final['out'])

    def test_list_ok(self):
        response1 = self.client.get('/agendamento/')
        self.client.post('/agendamento/', data=reg2['in'])
        self.client.post('/agendamento/', data=reg1['in'])
        ...
        self.assertEqual(
            response2.data['previous'],
            reg_list['out']['previous'])
        for i, reg in enumerate(reg_list['out']['results']):
            for item in reg:
                self.assertEqual(
                    response2.data['results'][i][item],
                    reg_list['out']['results'][i][item])

    def test_list_with_filter(self):
        self.client.post('/agendamento/', data=reg1['in'])
        self.client.post('/agendamento/', data=reg2['in'])
        self.client.post('/agendamento/', data=reg3['in'])
        response1 = self.client.get(
            '/agendamento/?min_data=%s&max_data=%s'
        ...
        self.assertEqual(
            response1.data['previous'],
            reg_list['out']['previous'])
        for i, reg in enumerate(reg_list['out']['results']):
            for item in reg:
                self.assertEqual(
                    response1.data['results'][i][item],
                    reg_list['out']['results'][i][item])
        ...
        self.assertEqual(
            response2.data['previous'],
            reg_list2['out']['previous'])
        for i, reg in enumerate(reg_list2['out']['results']):
            for item in reg:
                self.assertEqual(
                    response2.data['results'][i][item],
                    reg_list2['out']['results'][i][item])
        ...
        self.assertEqual(
            response3.data['previous'],
            reg_list2['out']['previous'])
        for i, reg in enumerate(reg_list2['out']['results']):
            for item in reg:
                self.assertEqual(
                    response3.data['results'][i][item],
                    reg_list2['out']['results'][i][item])
    ...
    def test_detail_ok(self):
        ...
        response1 = self.client.get(
            '/agendamento/%s/' % response_create1.data['id'])
        response2 = self.client.get(
            '/agendamento/%s/' % response_create2.data['id'])
